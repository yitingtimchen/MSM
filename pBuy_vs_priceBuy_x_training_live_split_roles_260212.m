        ...
        for f = 1:numel(C.eventTables)
            ...
            for role = roleList
                ...
                if role == "Subj"
                    ...
                    mkyField   = FIELD_monkey;
                else
                    optField   = FIELD_optionOpp;
                    trlField   = FIELD_trialNumOpp;
                    prcField   = FIELD_priceBuyOpp;
                    mkyField   = FIELD_monkeyOpp;
                end

                neededRole = {optField, trlField, prcField, mkyField};
                ...
                if isempty(etab.(mkyField){1})
                    continue;
                end
                monkeyStr = string(etab.(mkyField){1}(1));
...
for im = 1:numel(monkeys)
    ...
    % Plot per role: Subj = dark, Opp = light
    roleOrder = ["Subj","Opp"];
