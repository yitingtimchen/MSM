% Combined correlation analysis: Z_fr vs Opponent & Subject Portfolios
% BUY vs NON-BUY only; single figure with 2x2 subplots:
% [Opp Heatmap | Subj Heatmap; Opp Fisher z | Subj Fisher z]
% -----------------------------------------------------------------------------
clear; clc; close all;

% ========================== USER SETTINGS =====================================
% Path to condition packs
OUTDIR = 'C:\Users\plattlab\Tim\Stock_market_experiment\Tim\condition_packs_250922';
...
for s = 1:numel(cond_sets)
    ...
    if isempty(condNames), warning('No valid condition packs found.'); continue; end
    ...
    % ---- Keep trials that have neural data only (leave NaNs to 'Rows','complete')
    trialData = trialData(~isnan(trialData.Z_fr), :);
    ...
    % ==================== CORRELATIONS: BUY vs NON-BUY ONLY =================
    choice_labels = ["Buy","Non-buy"];

    % Opponent Portfolio
    rhos_opp      = zeros(numel(condition_grouped), 2); % BUY, NON-BUY
    pvals_rho_opp = zeros(numel(condition_grouped), 2);
    zs_opp        = zeros(numel(condition_grouped), 1);
    pvals_z_opp   = zeros(numel(condition_grouped), 1);

    % Subject Portfolio
    rhos_subj      = zeros(numel(condition_grouped), 2); % BUY, NON-BUY
    pvals_rho_subj = zeros(numel(condition_grouped), 2);
    zs_subj        = zeros(numel(condition_grouped), 1);
    ...
    for gp = 1:numel(condition_grouped)
        ...
        % --- Opponent portfolio correlations (pairwise-complete)
        [rhos_opp(gp,1), pvals_rho_opp(gp,1)] = corr(buy_trials.Z_fr,    buy_trials.opponentPortfolio, "Type","Pearson", "Rows","complete");
        [rhos_opp(gp,2), pvals_rho_opp(gp,2)] = corr(nonbuy_trials.Z_fr, nonbuy_trials.opponentPortfolio,"Type","Pearson","Rows","complete");

        % n used in each correlation (complete pairs)
        n_buy_opp    = sum(~isnan(buy_trials.Z_fr)    & ~isnan(buy_trials.opponentPortfolio));
        n_nonbuy_opp = sum(~isnan(nonbuy_trials.Z_fr) & ~isnan(nonbuy_trials.opponentPortfolio));

        % Fisher r-to-z: BUY vs NON-BUY (opponent)
        [zs_opp(gp), pvals_z_opp(gp)] = corr_diff_indep(rhos_opp(gp,1), n_buy_opp, rhos_opp(gp,2), n_nonbuy_opp, 'two');

        % --- Subject portfolio correlations (pairwise-complete)
        [rhos_subj(gp,1), pvals_rho_subj(gp,1)] = corr(buy_trials.Z_fr,    buy_trials.portfolio, "Type","Pearson", "Rows","complete");
        [rhos_subj(gp,2), pvals_rho_subj(gp,2)] = corr(nonbuy_trials.Z_fr, nonbuy_trials.portfolio,"Type","Pearson","Rows","complete");

        % n used in each correlation (complete pairs)
        n_buy_subj    = sum(~isnan(buy_trials.Z_fr)    & ~isnan(buy_trials.portfolio));
        n_nonbuy_subj = sum(~isnan(nonbuy_trials.Z_fr) & ~isnan(nonbuy_trials.portfolio));

        % Fisher r-to-z: BUY vs NON-BUY (subject)
        [zs_subj(gp), pvals_z_subj(gp)] = corr_diff_indep(rhos_subj(gp,1), n_buy_subj, rhos_subj(gp,2), n_nonbuy_subj, 'two');
    end

    % ==================== PLOTTING: ONE FIGURE, 2x2 SUBPLOTS ===============
    fh = figure('Color','w','Name',sprintf('%s | %s', setNames{s}, TARGET_EVT));
    ...
    % ---- (1,1) Opponent Portfolio Heatmap ----
    ax1 = subplot(2, 2, 1);
    imagesc(rhos_opp);
    colorbar;
    colormap(ax1, rwb);
    ...
    title(['Z_{fr} @ ' TARGET_EVT ' vs Opponent Portfolio (BUY vs NON-BUY)'], 'Interpreter','none');

    % Annotate cells
    [nRows_opp,nCols_opp] = size(rhos_opp);
    for i = 1:nRows_opp
        for j = 1:nCols_opp
            v = rhos_opp(i,j);
            p = pvals_rho_opp(i,j);
            ...
            text(j, i, lbl, 'HorizontalAlignment','center', 'FontWeight','bold', 'Color', tc, 'FontSize', 10);
        end
    end

    % ---- (1,2) Subject Portfolio Heatmap ----
    ax2 = subplot(2, 2, 2);
    imagesc(rhos_subj);
    colorbar;
    colormap(ax2, rwb);
    ...
    set(ax2,'XTick',1:numel(choice_labels),'XTickLabel',choice_labels, ...
        ...
        'TickDir','out','LineWidth',1);
    xlabel('Choice'); ylabel('Condition');
    title(['Z_{fr} @ ' TARGET_EVT ' vs Subject Portfolio (BUY vs NON-BUY)'], 'Interpreter','none');

    % Annotate cells
    [nRows_subj,nCols_subj] = size(rhos_subj);
    for i = 1:nRows_subj
        for j = 1:nCols_subj
            v = rhos_subj(i,j);
    ...
    % ---- (2,1) Fisher z: Opponent BUY - NON-BUY ----
    subplot(2, 2, 3); hold on
    bar(1:numel(condition_grouped_names), zs_opp);
    for x = 1:numel(condition_grouped_names)
        tc = 'k'; if pvals_z_opp(x) < 0.05, tc = 'r'; end
        text(x, zs_opp(x)+signOrOne(zs_opp(x))*0.3, pstars(pvals_z_opp(x)), ...
            'HorizontalAlignment','center', 'FontWeight','bold', 'Color', tc, 'FontSize', 12);
    end
    xticks(1:numel(condition_grouped_names))
    xticklabels(condition_grouped_names)
    ylabel("z")
    ylim([-8 8])
    title("Fisher r-to-z: Opponent (BUY - NON-BUY)")
    hold off

    % ---- (2,2) Fisher z: Subject BUY - NON-BUY ----
    subplot(2, 2, 4); hold on
    bar(1:numel(condition_grouped_names), zs_subj);
    for x = 1:numel(condition_grouped_names)
        tc = 'k'; if pvals_z_subj(x) < 0.05, tc = 'r'; end
        text(x, zs_subj(x)+signOrOne(zs_subj(x))*0.3, pstars(pvals_z_subj(x)), ...
            'HorizontalAlignment','center', 'FontWeight','bold', 'Color', tc, 'FontSize', 12);
    end
    xticks(1:numel(condition_grouped_names))
    xticklabels(condition_grouped_names)
    ylabel("z")
    ylim([-8 8])
    title("Fisher r-to-z: Subject (BUY - NON-BUY)")
    hold off

    sgtitle(sprintf('%s | %s', setNames{s}, TARGET_EVT), 'Interpreter','none')

    % Save combined figure
    exportgraphics(fh, fullfile(figdir, [TARGET_EVT '_fr_vs_portfolio_BUY_nonBUY_2x2_combined.svg']), 'Resolution', 300);
    savefig(fh,      fullfile(figdir, [TARGET_EVT '_fr_vs_portfolio_BUY_nonBUY_2x2_combined.fig']));

    % ---- Store outputs into allResults ----
...
if any([n1 n2] < 4) || any(abs([r1 r2])>=1) || any(isnan([r1 r2]))
z = NaN; p = NaN; return;
end
z1 = atanh(r1);
z2 = atanh(r2);
