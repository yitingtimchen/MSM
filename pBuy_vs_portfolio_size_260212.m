...
% Live role toggle:
%   true  -> include trials where the monkey is acting as opponent in Live (Role=="Opp")
%   false -> exclude Live opponent-role trials (analyze only Role=="Subj" everywhere)
INCLUDE_LIVE_OPPONENT_ROLE = 0;   % <-- TOGGLE
...
for s = 1:numel(cond_sets)
    ...
    for c = 1:numel(conds)
        ...
        for f = 1:numel(C.eventTables)
            ...
            % Role split: Live only (toggle controls whether to include Opp)
            if baseCond == "Live"
                if INCLUDE_LIVE_OPPONENT_ROLE
                    roleList = ["Subj","Opp"];
                else
                    roleList = "Subj";
                end
...
% Define sessionIdx per (Monkey, Session) so both roles share the same x-position
...
for im = 1:numel(monkeys)
    ...
    results(im).Monkey = mID;
    results(im).pGrid = pGrid;
    results(im).sessLo = sessLo;
    results(im).sessHi = sessHi;

    results(im).pBuyEarly = pBuyEarly;
    results(im).loBuyEarly = loBuyEarly;
    results(im).hiBuyEarly = hiBuyEarly;
    results(im).pBuyLate = pBuyLate;
    results(im).loBuyLate = loBuyLate;
    results(im).hiBuyLate = hiBuyLate;

    results(im).pSellEarly = pSellEarly;
    results(im).loSellEarly = loSellEarly;
    results(im).hiSellEarly = hiSellEarly;
    results(im).pSellLate = pSellLate;
    results(im).loSellLate = loSellLate;
    results(im).hiSellLate = hiSellLate;

    results(im).nEarlyByP = nEarly;
    results(im).nLateByP  = nLate;
end

% --- FIG 1 ---
figure('Color','w','Name','FIG 1: P(BUY | portfolio size) + occupancy (top/bottom)');
tl1 = tiledlayout(2, numel(monkeys), 'TileSpacing','compact', 'Padding','compact');

for im = 1:numel(monkeys)
    R = results(im);

    % TOP
    axTop = nexttile(tl1, im); hold(axTop,'on');
    ...
    title(axTop, sprintf('Monkey %s', string(R.Monkey)));
    ...
    % BOTTOM
    axBot = nexttile(tl1, numel(monkeys) + im); hold(axBot,'on');
...
drawnow;
for im = 1:numel(monkeys)
    axTop = nexttile(tl1, im);
    axBot = nexttile(tl1, numel(monkeys) + im);
...
tl2 = tiledlayout(2, numel(monkeys), 'TileSpacing','compact', 'Padding','compact');

for im = 1:numel(monkeys)
    R = results(im);

    % TOP
    axTop = nexttile(tl2, im); hold(axTop,'on');
    ...
    % BOTTOM
    axBot = nexttile(tl2, numel(monkeys) + im); hold(axBot,'on');
...
drawnow;
for im = 1:numel(monkeys)
    axTop = nexttile(tl2, im);
    axBot = nexttile(tl2, numel(monkeys) + im);
