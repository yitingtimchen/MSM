...
%   - Toggle: INCLUDE_LIVE_OPPONENT_ROLE (true/false)
...
% Toggle: include/exclude Live trials where the monkey is acting as opponent (Role=="Opp")
...
% --- Live opponent stream (Live only; used if INCLUDE_LIVE_OPPONENT_ROLE=true) ---
...
for s = 1:numel(cond_sets)
    ...
    for c = 1:numel(conds)
        ...
        for f = 1:numel(C.eventTables)
            ...
            if baseCond == "Live" && INCLUDE_LIVE_OPPONENT_ROLE
                roleList = ["Subj","Opp"];
            else
                roleList = "Subj";
            end
