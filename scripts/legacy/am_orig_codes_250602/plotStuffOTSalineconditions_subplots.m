...
cd C:\\Users\\plattlab\\MSM\\data\\tooling\\MATLAB_Copy\\MSM_SpikeData\Saline\AI_Saline
load('m1rew1on.mat')
aiUnits_Saline = spikes;
aiNames_Saline = units;
cd C:\\Users\\plattlab\\MSM\\data\\tooling\\MATLAB_Copy\\MSM_SpikeData\Saline\Replay_Saline
load('m1rew1on.mat')
replayUnits_Saline = spikes;
replayNames_Saline = units;
cd C:\\Users\\plattlab\\MSM\\data\\tooling\\MATLAB_Copy\\MSM_SpikeData\Saline\Decoy_Saline
load('m1rew1on.mat')
decoyUnits_Saline = spikes;
decoyNames_Saline = units;
cd C:\\Users\\plattlab\\MSM\\data\\tooling\\MATLAB_Copy\\MSM_SpikeData\Saline\Live_Saline
load('m1rew1on.mat')
...
data(:,:,1) = aiUnits_OT(:,1:57);
data(:,:,2) = replayUnits_OT(:,1:57);
data(:,:,3) = decoyUnits_OT(:,1:57);
data(:,:,4) = liveUnits_OT(:,1:57);
data(:,:,5) = aiUnits_Saline(:,1:57);
data(:,:,6) = replayUnits_Saline(:,1:57);
data(:,:,7) = decoyUnits_Saline(:,1:57);
data(:,:,8) = liveUnits_Saline(:,1:57);


time = -0.5:0.005:1.495; % seconds, sampled at 5 Hz

colors1 = cbrewer2('div', 'BrBG', 4);
colors2 = cbrewer2('div', 'RdYlBu', 4);

colors(1:2,:) = [colors1(1,:);colors2(1,:)]; %too tired to do anything but the long, stupid way.
colors(3:4,:) = [colors1(2,:);colors2(2,:)];
colors(5:6,:) = [colors1(3,:);colors2(3,:)];
colors(7:8,:) = [colors1(4,:);colors2(4,:)];

 
subplot(4,1,1);  % plot across four subplots
%--- Subplot 1
hold on;
bl = boundedline(time, aiMean_OT, SEM_ai_OT, ...
    time, aiMean_Saline, SEM_ai_Saline, ...
    'cmap', colors);
...
% --- Subplot 2
subplot(4,1,2);  % plot across four subplots
b2 = boundedline(time, replayMean_OT, SEM_replay_OT, ...
    time, replayMean_Saline, SEM_replay_Saline, ...
    'cmap', colors);

% Add lines
h4 = line([-0.5 -0.5],[-1.5 2.5]);
% Set properties of lines
set(h4,'Color','k','LineWidth',0.75)

%----- Subplot 3
subplot(4,1,3);  % plot across four subplots
b3 = boundedline(time, decoyMean_OT, SEM_decoy_OT, ...
    time, decoyMean_Saline, SEM_decoy_Saline, ...
    'cmap', colors);

% Add lines
h5 = line([-0.5 -0.5],[-1.5 2.5]);
% Set properties of lines
set(h5,'Color','k','LineWidth',0.75)


% --- subplot 4
subplot(4,1,4);  % plot across four subplots
b4 = boundedline(time, liveMean_OT, SEM_live_OT, ...
    time, liveMean_Saline, SEM_live_Saline, ...
    'cmap', colors);

% Add lines
h6 = line([-0.5 -0.5],[-1.5 2.5]);
% Set properties of lines
set(h6,'Color','k','LineWidth',0.75)
 
% instead of a legend, show colored text
lh = legend(bl,b2,b3,b4);
legnames = {'AI (OT)','AI (Saline)','Replay (OT)','Replay (Saline)',...
    'Decoy (OT)','Decoy (Saline)','Live (OT)','Live (Saline)'};
