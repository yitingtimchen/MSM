% File: d20260212_analyze_optimal_choice.m
...
[G, keyMonkey, keySession, keyTreatment, keyConditionBase, keyLiveFlag, keyRole] = findgroups( ...
    trialTableAll.Monkey, trialTableAll.AnalysisSession, trialTableAll.Treatment, trialTableAll.ConditionBase, trialTableAll.LiveFlag, trialTableAll.Role);
...
sessionSummaryAll = table(keyMonkey, keySession, keyTreatment, keyConditionBase, keyLiveFlag, keyRole, ...
    ...
    'VariableNames', {'Monkey','AnalysisSession','Treatment','ConditionBase','LiveFlag','Role', ...
                      'nTrials','pBestDP','pBestFV','pBenchmarksDisagree'});

%% -------------------------- sessionIdx per monkey -----------------------------
% In LIVE_ROLE_MODE=="combined", session keys are role-qualified in Live rows,
% so session progression doubles for Live within each monkey.
sessTab = unique(sessionSummaryAll(:, {'Monkey','AnalysisSession'}));
[sessTabSorted, sortIdxSess] = sortrows(sessTab, {'Monkey','AnalysisSession'});
