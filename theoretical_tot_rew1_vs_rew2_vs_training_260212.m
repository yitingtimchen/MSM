    ...
    for c = 1:numel(conds)
        ...
        for f = 1:numel(C.eventTables)
            ...
            for role = roleList
                ...
                newRow = table( ...
                    string(treatName), string(cname), string(baseCond), string(role), string(actorMonkeyStr), sessionStr, sessionDate, ...
                    ...
                    'VariableNames', { ...
                        'Treatment','ConditionRaw','ConditionBase','Role','Monkey','Session','SessionDate', ...
...
sessionSummaryAll.Role   = categorical(string(sessionSummaryAll.Role), ["Subj","Opp"]);

% sessionIdx: chronological order within each monkey (shared across roles)
sessTab = unique(sessionSummaryAll(:, {'Monkey','Session'}));
[sessTabSorted, ~] = sortrows(sessTab, {'Monkey','Session'});
...
% join back so both roles share same x coordinate
...
plot_metric_vs_x_ROLE(sessionSummaryAll, monkeys, treatCats, baseCats, baseColors, markerList, ...
    yField, yLabel, 'idx', sg1Title);

figure('Name', fig2Name, 'Color','w');
plot_metric_vs_x_ROLE(sessionSummaryAll, monkeys, treatCats, baseCats, baseColors, markerList, ...
    yField, yLabel, 'date', sg2Title);
...
plot_metric_vs_x_ROLE(sessionSummaryAll, monkeys, treatCats, baseCats, baseColors, markerList, ...
    ...
    'Mean post-choice portfolio over training sessions (sessionIdx, all treatments)');

figure('Name', 'Portfolio trend vs training (by date)', 'Color','w');
plot_metric_vs_x_ROLE(sessionSummaryAll, monkeys, treatCats, baseCats, baseColors, markerList, ...
    portfolioField, portfolioLabel, 'date', ...
    'Mean post-choice portfolio over training sessions (session date, all treatments)');
