...
for r = 1:3
    ...
    for im = 1:numel(monkeys)
        ...
        % scatter by Treatment (shape) and ConditionBase (fill color)
        % Live is split by Role: Subj (dark) vs Opp (light)
...
% Live is split into Subj (dark) vs Opp (light)
condHandles = gobjects(numel(baseCondList)+1,1);
condLabels  = strings(numel(baseCondList)+1,1);
idx = 1;
